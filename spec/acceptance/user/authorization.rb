...
feature 'Authorization by oauth provider', %q{
  In order to authorize using social networks login
  ...
  describe 'Sign in with valid credentials' do
    scenario 'Sign in with facebook' do
      ...
      expect(page).to have_content 'Successfully authenticated through Facebook account.'
    end
    ...
    scenario 'Sign in with Twitter' do
      ...
      expect(page).to have_content 'Successfully authenticated through Twitter account.'
    end
  end
